...
public class DirtyObject : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if(DirtyObjectManager.Instance)DirtyObjectManager.Instance.AddDirtyObject(this);
    }
    ...
    private void Bleeding_GauzePads()
    {
        // Debug.Log(_typeOfCleaner + " huh " + isBleeding + " ?? " + isDoneCleaning);
        if(_typeOfCleaner == ETypeOfCleaner.GauzePads && isBleeding && !isDoneCleaning)
        {
            // Debug.Log("Wat???");
            if(curGauzePadTime < MAX_POINT)
            {
                // Debug.Log("Masuk sini");
                if(bleedCountdown > 0)
                {
                    // Debug.Log("Masuk sinis");
                    bleedCountdown -= 1;
                }
                else
                {
                    // Debug.Log("Trakhir sinis");
                    curGauzePadTime += _bleedingMultiplier;
                    if(curGauzePadTime >= MAX_POINT)curGauzePadTime = MAX_POINT;
    ...
    public void UnRegisterCleaner(DirtyCleaner cleaner)
    {
    ...
    {
        if (curTransparancy < 0f && !isDoneCleaning)
        {
